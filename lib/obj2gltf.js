 ...
 * @param {String} [options.offsetFile] A three line file that contains vertex position offsets
 * @param {Boolean} [options.transformVertices=false] Transform vertex positions using proj4.
 * @param {String} [options.sourceProjection] Source projection for transforming vertex positions. Should be a proj or wkt string.
 * @param {String} [options.destProjection] Destination projection for transforming vertex positions. Should be a proj or wkt string.
...
function obj2gltf(objPath, options) {
    ...
    options.offsetFile = defaultValue(options.offsetFile, defaultValue.EMPTY_OBJECT);
    options.transformVertices = defaultValue(options.transformVertices, defaults.transformVertices);
    options.sourceProjection = defaultValue(options.sourceProjection, defaults.sourceProjection);
    options.destProjection = defaultValue(options.destProjection, defaults.destProjection);
...
obj2gltf.defaults = {
    ...
    /**
     * Gets or sets whether the vertex positions in the OBJ file will be transformed using proj4
     * @type Boolean
     * @default false
     */
    ...
    /**
     * Gets or sets the source projection for transforming vertex positions
     * @type String
     * @default UTM
     */
    sourceProjection : '+proj=utm +zone=15 +ellps=clrk66 +units=m +no_defs',

    /**
     * Gets or sets the destination projection for transforming vertex positions
     * @type String
     * @default latlong
     */
    destProjection : '+proj=longlat +ellps=WGS84 +datum=WGS84 +no_defs'
};
