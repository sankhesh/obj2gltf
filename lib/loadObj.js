...
function loadObj(objPath, options) {
    ...
    var offsets = new ArrayStorage(ComponentDatatype.FLOAT);
    ...
    function parseLine(line) {
        ...
        if ((line.length === 0) || (line.charAt(0) === '#')) {
            ...
            if (offsets.length > 0) {
                positions[positions.length - 3] += offsets[0];
                positions[positions.length - 2] += offsets[1];
                positions[positions.length - 1] += offsets[2];
            }
