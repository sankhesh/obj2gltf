...
function loadObj(objPath, options) {
    ...
    function parseOffsetLine(line) {
      ...
      if ((line.length === 0) || (line.charAt(0) === '#')) {
          ...
          offsets.push(parseFloat(line));
      }
    }

    if (options.offsetFile.length > 0) {
        readLines(options.offsetFile, parseOffsetLine);
    }
    ...
    function parseLine(line) {
        ...
        if ((line.length === 0) || (line.charAt(0) === '#')) {
            ...
            var i;
            for (i = 0; i < 3; ++i) {
                tempPos[i] = parseFloat(result[i+1]);
                if (offsets.length > i) {
                    tempPos[i] += offsets[i];
                }
            }
            if (options.transformVertices) {
                var transformedPos = proj(options.sourceProjection,
                                          options.destProjection,
                                          tempPos);
                for (i = 0; i < 2; ++i) {
                    tempPos[i] = transformedPos[i];
                }
            }
            for (i = 0; i < 3; ++i) {
                positions.push(tempPos[i]);
            }
